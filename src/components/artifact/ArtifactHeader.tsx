...
export function ArtifactHeader({ artifact, onDelete, onNameChange }: ArtifactHeaderProps) {
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const [isEditing, setIsEditing] = useState(false);
  const [editName, setEditName] = useState(artifact.metadata.name || '');
  ...
  const handleSaveName = () => {
    ...
    if (trimmedName !== artifact.metadata.name && onNameChange) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      setEditName(artifact.metadata.name || '');
      setIsEditing(false);
    }
  };
          ...
          <div className="flex items-center gap-3 mb-1">
            {isEditing ? (
              <input
                ...
                placeholder={t('artifact.untitled')}
                className="text-xl sm:text-2xl text-obsidian-50 bg-transparent border-b-2 border-gold-500 outline-none px-1 -mx-1 min-w-[120px]"
              />
            ) : (
              <button
                type="button"
                onClick={() => setIsEditing(true)}
                className="text-xl sm:text-2xl text-obsidian-50 hover:text-gold-400 transition-colors flex items-center gap-2 group"
                title={t('artifact.editName') || 'Click to edit name'}
              >
                {artifact.metadata.name || t('artifact.untitled')}
                <EditIcon className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity text-gold-500" />
              </button>
            )}
